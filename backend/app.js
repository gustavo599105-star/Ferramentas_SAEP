...
server.put('/produtos/:id' , (req, res) => {
    ...
    const sql = `UPDATE PRODUTO SET nome = ?, cor = ?, textura = ?,
        peso = ?, unidade_medida = ?, aplicacao = ?, 
        data_validade = ?, estoque_minimo = ?, 
        estoque_atual = ?, preco_unitario = ?, id_categoria = ? WHERE id_produto = ?`
    
    connection.query(sql, [ nome, cor, textura, peso, 
        unidade_medida, aplicacao, 
        data_validade, estoque_minimo, 
        estoque_atual, preco_unitario, id_categoria, id] , (erro) => {
            if(erro){
                return res.status(500).json({erro: erro.message})
            }
